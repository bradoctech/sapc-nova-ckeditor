    ...
    >
                            ...
                            <optgroup :label="__('Ordenar por')">
                                ...
                                <template v-else>
                                    ...
                                    <template v-if="hasLaruploadTrait">
                                        <option value="file_file_size">
                                            {{ __("Tamanho") }}
                                        </option>
                                        <option value="file_file_width">
                                            {{ __("Largura") }}
                                        </option>
                                        <option value="file_file_height">
                                            {{ __("Altura") }}
                                        </option>
                                        <option value="file_file_duration">
                                            {{ __("Duração") }}
                                        </option>
                                    </template>
        ...
        <transition name="image-loading" mode="out-in">
            <div
                ...
                class="media-main h-full overflow-hidden"
                :class="{ active: dragCounter > 0 }"
            >
                <div
                    v-if="isUploading"
                    class="flex flex-col h-full text-primary-600 content-center justify-center text-center overflow-hidden"
                >
                    <div
                ...
                >
                    <div class="grid nc-grid-cols-6 gap-4 mb-12 mt-6 px-6">
                        <div
                            ...
                            :key="item.hash"
                            @click="select(item)"
                            class="media-container text-center p-1 cursor-pointer"
                            :title="item.name"
                        >
                            <div
                            ...
                            >
                                <div
                                    @click.stop.prevent="directSelect(item)"
                                    class="direct-select"
                                    :title="__('Direct Select')"
                                >
                                    <Icon type="plus" />
                                </div>
                            ...
                            <strong class="media-name text-primary-600">{{
                                item.name
                            }}</strong>
                        </div>
                    </div>
                ...
                <div
                    v-else
                    class="flex flex-col h-full text-primary-600 content-center justify-center text-center"
                >
                    <p>{{ __("Nenhum resultado.") }}</p>
                    <span class="block">{{
                        __("Solte seus arquivos aqui")
                    }}</span>
                </div>
            </div>
        </transition>
                ...
                <div class="flex">
                    <div>
                        <button
                            ...
                            class="bg h-9 shadow bg-primary-500 hover:bg-primary-400 text-white dark:text-gray-900 cursor-pointer rounded inline-flex items-center justify-center px-3 shadow relative"
                        >
                            <span v-if="selected.length < 2">{{
                                __(pickerLabel)
                            }}</span>
                            <span v-else-if="isImagePicker">{{
                                __("Insert :count Images", {
                                    count: selected.length,
                                })
                            }}</span>
                            <span v-else-if="isVideoPicker">{{
                                __("Insert :count Videos", {
                                    count: selected.length,
                                })
                            }}</span>
                            <span v-else>{{
                                __("Insert :count Audios", {
                                    count: selected.length,
                                })
                            }}</span>
                        </button>
                    ...
                    <div
                        class="inline-flex flex-grow text-primary-600 self-center items-center ml-3"
    ...
    computed: {
        ...
        title() {
            if (this.isImagePicker) {
                return "Seleção de imagem";
            }
            if (this.isVideoPicker) {
                return "Seleção de video";
            }

            return "Seleção de áudio";
        },

        searchPlaceholder() {
            if (this.isImagePicker) {
                return "Pesquisar Imagem ...";
            }
            if (this.isVideoPicker) {
                return "Pesquisar Vídeo ...";
            }

            return "Pesquisar Áudio ...";
        },
    ...
    methods: {
        ...
        async fetch(page = 0) {
            ...
            return await this.fetchResourceCollection(this.resourceKey, params)
                .then((entities) => {
                    this.items =
                        newPage > 1 ? this.items.concat(entities) : entities;
        ...
        handleUploads({ dataTransfer }) {
            ...
            const requests = [...dataTransfer.files].map((file) => {
                ...
                return this.uploadResource(this.resourceKey, data).then(
                    (item) => {
                        if (item && item.name) {
                            Nova.success(
                                this.__("File uploaded: :file", {
                                    file: item.name,
                                })
                            );
        ...
        displayIntro() {
            if (!this.intro) {
                ...
                const content = this.__(
                    "You can upload your files by dragging them here"
                );
